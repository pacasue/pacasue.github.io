...
`),n=``,r,i=[];for(let e of t){let t=e.match(/^title:\s*(.+)$/),a=e.match(/^tag:\s*(.+)$/);if(t){n=t[1].trim();continue}if(a){r=a[1].trim();continue}i.push(e)}return{title:n,tag:r,body:i.join(` `).trim()}}),r=l.length;return l.push({cards:n}),`CARD_GRID_PLACEHOLDER_${r}`}),o=o.replace(/:::case-study-separator:::/g,`CASE_STUDY_SEPARATOR`),o=o.replace(/:::clinical-trial:::/g,`CLINICAL_TRIAL`),(0,j.jsx)(`article`,{className:`max-w-2xl`,children:o.split(/\n\n+/).map((e,o)=>{let u=e.trim();if(!u)return null;if(u===`CASE_STUDY_SEPARATOR`)return(0,j.jsx)(ki,{},o);if(u===`CLINICAL_TRIAL`)return(0,j.jsx)(Li,{},o);let d=u.match(/^REFERENCES_PLACEHOLDER_(\d+)$/);if(d){let e=c[parseInt(d[1])];return(0,j.jsx)(Ai,{items:e.items},o)}let f=u.match(/^BARCHART_PLACEHOLDER_(\d+)$/);if(f){let e=r[parseInt(f[1])];return(0,j.jsx)(Ei,{title:e.title,bars:e.bars,footnote:e.footnote},o)}let p=u.match(/^LINECHART_PLACEHOLDER_(\d+)$/);if(p){let e=i[parseInt(p[1])];return(0,j.jsx)(Di,{title:e.title,xLabel:e.xLabel,yLabel:e.yLabel,series:e.series,points:e.points,footnote:e.footnote},o)}let m=u.match(/^SPLIT_TABLE_PLACEHOLDER_(\d+)$/);if(m){let e=s[parseInt(m[1])];return(0,j.jsx)(Oi,{left:e.left,right:e.right},o)}let h=u.match(/^CAROUSEL_PLACEHOLDER_(\d+)$/);if(h){let e=t[parseInt(h[1])];return(0,j.jsx)(Ci,{quotes:e.quotes},o)}let g=u.match(/^IMAGEROW_PLACEHOLDER_(\d+)$/);if(g){let e=n[parseInt(g[1])];return(0,j.jsx)(wi,{images:e.images},o)}let _=u.match(/^CASCADE_LIST_PLACEHOLDER_(\d+)$/);if(_){let e=a[parseInt(_[1])];return(0,j.jsx)(ji,{items:e.items},o)}let v=u.match(/^CARD_GRID_PLACEHOLDER_(\d+)$/);if(v){let e=l[parseInt(v[1])];return(0,j.jsx)(Mi,{cards:e.cards},o)}let y=u.match(/^(\*\*CTA:\*\*|CTA:)\s*(.+)$/s);if(y)return(0,j.jsx)(Si,{children:N(y[2])},o);let b=u.match(/^## (.+)/);if(b)return(0,j.jsx)(mi,{id:Pi(b[1]),children:b[1]},o);let x=u.match(/^!\[([^\]]*)\]\(([^)]*)\)$/);if(x){let e=x[1].endsWith(`|full`),t=e?x[1].slice(0,-5):x[1];return x[2]?(0,j.jsx)(xi,{src:x[2],caption:t,full:e},o):(0,j.jsx)(Ti,{caption:t},o)}let ee=u.match(/^### (.+)/);if(ee)return(0,j.jsx)(hi,{id:Pi(ee[1]),children:ee[1]},o);let S=u.match(/^#### (.+)/);if(S)return(0,j.jsx)(gi,{id:Pi(S[1]),children:S[1]},o);let C=u.match(/^# (.+)/);if(C)return(0,j.jsx)(mi,{id:Pi(C[1]),children:C[1]},o);if(u.startsWith(`- `)||u.startsWith(`* `))return(0,j.jsx)(`ul`,{className:`list-disc list-inside text-charcoal-300 text-base md:text-[17px] leading-[1.85] mb-5 space-y-1 pl-2`,children:u.split(`
